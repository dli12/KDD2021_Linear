...
# Load fullrank similirity matrix and sparsification
z = np.load('fullrank.npz')
BBB = z['BB']
mask = (1. * (np.abs(BBB)>0.001))
BBB = BBB * mask
...
class SparseModel(nn.Module):
    ...
    def __init__(self, num_item, BBB, UI, p):
        ...
        self.W = nn.Embedding(num_item, num_item)
    ...
    def getW(self):
        ...
        return self.BBB * (torch.sigmoid(self.W.weight))
    
    
    def bpr_loss2(self, user, pos_item, neg_item):
        
        #A = torch.tensor(self.UI[user]).float().to(device)
        #A = torch.tensor(self.UI[user].toarray()).float().to(device) # for sparse
        A = self.UI[user] # for GPU tensor
...
W = torch.tensor(BBB).float().to(device)



model = SparseModel(n_items, W, UI, p).to(device)

optimizer = torch.optim.Adam(model.parameters(), lr = lr)



fname = "fullrank_sparse"
...
for epoch in range(epochs):
    ...
    t1 = time.time()
    f = open("save/" + fname + ".txt", "a")
    f.write('sample take time %f'%(t1-t0))
    f.write("\n")
    f.close()
    ...
    for idx, batch_data in enumerate(train_dataloader):
        ...
        #user = batch_data['user'].long().to(device) # for GPU
        user = batch_data['user'].long().numpy()
        ...
        if (idx+1)%5000 == 0:
            ...
            f = open("save/"+fname + ".txt", "a")
            f.write('5000 epoch take time: %f'%(time.time() - t1))
            f.write("\n")
